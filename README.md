...
1. Change in `./nginx/nginx.conf` the line with *proxy_pass* from `proxy_pass http://localhost:${PORT};` to `proxy_pass http://flask_app:${PORT};` in order to make it work only on Docker.
